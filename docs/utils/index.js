...
async function getInfoFromUrl(link) {
  ...
  try {
    ...
    // 提取所有 meta 标签的属性
    const allMetas = Array.from(htmlPage.childNodes).filter(
    ...
    title =
      htmlPage.querySelector("title").textContent ||
      getMetaValue("name", "og:title") ||
      getMetaValue("property", "og:title");

    description =
      getMetaValue("name", "description") ||
      getMetaValue("name", "og:description") ||
      getMetaValue("property", "og:description") ||
      link;

    const firstImg = htmlPage.querySelector("img");
    images =
      getMetaValue("name", "image") ||
      getMetaValue("property", "image") ||
      getMetaValue("name", "og:image") ||
      getMetaValue("property", "og:image") ||
      getMetaValue("name", "twitter:image") ||
      getMetaValue("property", "twitter:image") ||
      htmlPage.querySelector("link[rel=icon]")?.href ||
      htmlPage.querySelector("link[apple-touch-icon]")?.href ||
...
export default function transformLinkToCard(documentEle) {
  ...
  setTimeout(() => {
    const cardLinks = Array.from(
      documentEle?.querySelectorAll?.("a[link=card]")
    );
